...
```
• Built end-to-end customer analytics pipeline in Python (pandas, scikit-learn, matplotlib/seaborn)
  analysing 8,500 transactions across 2,000 customers to drive retention strategy.

• Engineered RFM features and applied K-Means clustering to create 5 actionable customer segments;
  Champions segment contributed 51% of total revenue despite being 20% of the customer base.

• Trained Random Forest churn classifier (ROC-AUC 1.00) identifying recency as the dominant
  predictor; recommended targeted win-back campaigns for 88 At-Risk customers worth $82K in CLV.

• Designed cohort retention heatmap and monthly retention curve revealing a 45% average retention rate,
  enabling data-driven targeting of lapsed cohorts.

• Produced 10-chart automated dashboard (PNG exports) aligned to a Power BI specification with
  20 visuals, 8 KPI cards, and DAX measures for stakeholder reporting.
```
